...
export default function FindVetsNearby() {
	...
	useEffect(() => {
		...
		axios
			.get(apiLink + "/get-vet-clinic/", { params: { user_id: uid } })
			.then((response) => {
	...
	const handleDelete = async (shelterId) => {
		try {
			...
			const response = await axios.post(
				apiLink + "/delete-vet-clinic/",
				new URLSearchParams({
					user_id: uid,
					vet_clinic_id: shelterId,
				}),
		...
		try {
			const response = await axios.post(
				...
				new URLSearchParams({
					...
					vet_clinic_id: selectedShelter.id,
				}),
				{ headers: { "Content-Type": "application/x-www-form-urlencoded" } }
			);
								...
								>
									...
									{latitude && longitude && (
										<LeafletMarker position={[latitude, longitude]}>
											<Popup>
												{shelter.name} <br /> {shelter.address}
											</Popup>
										</LeafletMarker>
									)}
								</MapContainer>
